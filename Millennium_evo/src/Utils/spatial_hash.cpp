...
public:
    ...
    void computeFOV(vector<Entity*>& entities, int window_w, int window_h){
        for (auto e : entities) {
            if(!e->active) continue;
            if(!e->active) continue;
            if(Ship* s = dynamic_cast<Ship*>(e)){
                ...
                //if(s->position.x + s->fovradius > window_w || s->position.x - s->fovradius < 0 
                //|| s->position.y + s->fovradius > window_h || s->position.y - s->fovradius < 0){
                //    if (s->check_fov_border()) {
                //            s->set_fov_detect(1);
                //            break;
                //        }
                //}
