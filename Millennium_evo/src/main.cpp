...
int main() {
    ...
    for(int i = 0; i < nIndv; i++) {
        ...
        population.emplace_back(ship_phys, GENOME_SIZE, 0, 0, screenHeight, screenWidth, textures[2], i); //coloca o cerebro na nave
    }
    ...
    while (!WindowShouldClose()) {
        ...
        if (timer >= gen_duration || alive == 0) {
            ...
            // 3. Reset World
            world.Reset(false); // true = keep asteroids, erase ships
            for(int i = 0; i < nAsteroids; i++) world.Spawn_entity(1);
            ...
            for(int i = 0; i < nIndv; i++) {
                ...
                if(i == 0) ship_phys->eh_o_melhor = true;
                population.emplace_back(ship_phys, next_genomes[i], 0, 0, screenHeight, screenWidth, textures[2], i);
            }
